...
│  └─ types.ts              # 타입 정의
├─ data/
│  └─ questionPool.json     # 문제 풀 (30개 템플릿)
...
**Q. 동점자 처리?**
A. 공동 순위 (1위 2명이면 둘 다 1위, 다음은 3위).

**Q. 문제당 몇 개?**
A. `questionPool.json`에 30개 템플릿 준비됨. 방 생성 시 전체 풀에서 원하는 수만큼 랜덤 추출.
...
- [ ] `questionPool.json`에 상품 최소 15개 이상 실제 데이터 입력
